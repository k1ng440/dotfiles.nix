...
in {
  ...
  # Implementation
  config.wayland.windowManager.hyprland.extraConfig = let
    renderSingleBind = n: v: let
      hasDesc = assert assertMsg (!(hasInfix "," (v.description or "")))
      ...
      checkAndNormalizeModifier = x:
        pipe x [
          toUpper
          (y:
            assert assertMsg (elem y ["SHIFT" "CTRL" "ALT"])
            "Unknown modifier: ${y}"; y)
        ];

      n' = pipe n [(splitString "+") last];
      mod' = concatStringsSep " " ([v.mod or ""]
        ++ (
          if nameHasMod
          then
            pipe n [
    ...
    in
      pipe [mod' n' (v.description or null) v.dispatcher (v.arg or null)] [
    ...
    removeMetaArgs =
      flip removeAttrs ["dispatcher" "arg" "description" "mod"];

    recursiveRender = outer:
    # Looks like nested submaps need one and only one "submap=reset"
    attrset:
      pipe attrset [
        (mapAttrs (n: v: let
          ...
          v' =
            if
              ...
              &&
              # Has subkeys
              builtins.length (pipe v [removeMetaArgs builtins.attrValues])
              > 0
            then v // {dispatcher = "submap";}
            else v;
        in
          if v'.dispatcher == "submap" && v'.arg != "reset"
          then
            (pipe v' [
              # Drop meta arguments
              removeMetaArgs
