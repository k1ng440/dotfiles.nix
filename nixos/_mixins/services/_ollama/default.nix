...
let
  ...
  # Construct URLs for each host in accelerationMap, excluding unknown
  baseUrls = lib.concatStringsSep ";" (
    map (host: "http://${host}:11434") (
      lib.filter (host: host != "unknown") (builtins.attrNames accelerationMap)
    )
  );
in
{
  environment = {
    ...
    systemPackages = lib.mkIf hasAcceleration (
      with pkgs;
      [
        unstable.gollama
      ]
